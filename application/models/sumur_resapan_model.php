...
class Sumur_resapan_model extends CI_Model {
   ...
   //select 
	...
	#data usulan
		public function getJumlahsumur_resapanAwal() {
			 $sql = "SELECT *
					FROM `sumur_resapan` ";
			return $this->db->query($sql);
		}
    ...
    public function select_all_verifikasi() {
        $sql = "select * from sumur_resapan where status_data_awal='1' and status_verifikasi='1' and status_sedang_dilaksanakan='0' and status_sudah_dilaksanakan='0'  and status_tidak_dilaksanakan='0'";
        return $this->db->query($sql);
    }

    //select
    public function select_all_sedang_dilaksanakan() {
        $sql = "select * from sumur_resapan where status_data_awal='1' and status_verifikasi='1' and status_sedang_dilaksanakan='1' and status_sudah_dilaksanakan='0'  and status_tidak_dilaksanakan='0'";
        return $this->db->query($sql);
    }

    //select
    public function select_all_sudah_dilaksanakan() {
        $sql = "select * from sumur_resapan where status_data_awal='1' and status_verifikasi='1' and status_sedang_dilaksanakan='1' and status_sudah_dilaksanakan='1'  and status_tidak_dilaksanakan='0'";
        return $this->db->query($sql);
    }
    ...
    public function select_by_id($id_sumur_resapan) {
        $sql = " select * from sumur_resapan where id_sumur_resapan=" . $id_sumur_resapan;
        $query = $this->db->query($sql);
        return $query;
    }

    public function search_all($keyword) {
        $sql = "select * from sumur_resapan where username like '%" . $keyword . "%'";
        return $this->db->query($sql);
    }
