...
class Users_model extends CI_Model{
    ...
    public function get_user_by_id($id_pengguna) {
        $sql = "select u.*, us.jenis_pengguna as jenis_pengguna, s.state as state, c.country_name as country_name 
                from pengguna u, jenis_pengguna us, states_tabel s, country_tabel c 
                where u.id_jenis_pengguna = us.id_jenis_pengguna and c.id_country = u.id_country and s.id_state = u.id_state and u.id_pengguna = '$id_pengguna'";
        $query = $this->db->query($sql);
        if ($query->num_rows() == 1){
            return $query->row_array();
        }
