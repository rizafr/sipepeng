<?php
	...
	class Artesis_managements extends CI_Controller {
		var $gallery_path;
		...
		public function index() {
			...
			if($this->session->userdata('is_login')){
				...
				//status 1 = data awal
				if($status=='1'){       	
					$data['artesis_list'] = $this->artesis_model->select_all_data_awal()->result();
				}
				
				//status 2 = data verifikasi
				if($status=='2'){       	
					$data['artesis_list'] = $this->artesis_model->select_all_verifikasi()->result();
				}
				
				//status 3 = data progress
				if($status=='3'){       	
					$data['artesis_list'] = $this->artesis_model->select_all_sedang_dilaksanakan()->result();
				}
				
				//status 4 = data sudah dilaksanakan
				if($status=='4'){       	
					$data['artesis_list'] = $this->artesis_model->select_all_sudah_dilaksanakan()->result();
				}
				
				//status 5 = data tidak dilaksanakan
				if($status=='5'){       	
					$data['artesis_list'] = $this->artesis_model->select_all_tidak_dilaksanakan()->result();
				}		
				
				$data['status']= $status;
				$this->load->view('admin/artesis/artesis_list',$data);
		...
		public function gotoForm() {
			...
			if($this->session->userdata('is_login')){
				...
				$data['title'] = "Data Awal Artesis | SIPEPENG";
				$data['judulForm'] = "Data Awal Artesis";
				$data['username'] = $this->session->userdata('username');
				...
				if($data['aksi']=='edit'){
				//mengambil uri aksi
				$id_artesis =$this->uri->segment(5);
				$data['artesis_list'] = $this->artesis_model->getArtesisById($id_artesis);
				}
				
				$this->load->view('admin/artesis/artesis_olahdata', $data);
			}else {
				redirect('public/homes');
			}
		}
		...
		public function process() {
			...
			$data['aksi']=$aksi;
			$id_artesis = $this->input->post('id_artesis');
			...
			if ($this->form_validation->run() == TRUE) {						
				...
				if($aksi=='add'){
					...
					$hasil = $this->artesis_model->add($data);
				...
				$data['username'] = $this->session->userdata('username');
				redirect('admin/artesis_managements/index/1',$data);
		...
		function delete($id_artesis) {
			$id_artesis=$this->uri->segment(4);
			$status=$this->uri->segment(5);
			if (empty($id_artesis)) {
				$this->session->set_flashdata('message', 'Error Invalid');
				redirect('admin/artesis_managements/index/'.$status);
				} else {
				$this->artesis_model->delete($id_artesis);
		...
		public function view($id_artesis){
		...
		function update_status_data_awal(){
			$id_artesis=$this->uri->segment(4);
			$hasil = $this->artesis_model->update_status_data_awal($id_artesis);
			...
			redirect('admin/artesis_managements/index/1');
		}
		
		function update_status_verifikasi(){
			$id_artesis=$this->uri->segment(4);
			$hasil = $this->artesis_model->update_status_verifikasi($id_artesis);
			//message berhasil loncat
			...
			redirect('admin/artesis_managements/index/2');
		}
		function update_status_sedang_dilaksanakan(){
			$id_artesis=$this->uri->segment(4);
			$hasil = $this->artesis_model->update_status_sedang_dilaksanakan($id_artesis);
			//message berhasil loncat
			...
			redirect('admin/artesis_managements/index/3');
		}
		function update_status_sudah_dilaksanakan(){
			$id_artesis=$this->uri->segment(4);
			$hasil = $this->artesis_model->update_status_data_awal($id_artesis);
			redirect('admin/artesis_managements/index/4');
		}
		function update_status_tidak_dilaksanakan(){
			$id_artesis=$this->input->post('id');
			$hasil = $this->artesis_model->update_status_tidak_dilaksanakan($id_artesis);
			//message berhasil loncat
