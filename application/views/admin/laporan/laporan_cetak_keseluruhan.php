...
<?php
	...
	$nama_file= "Laporan Cetak Keseluruhan.xls";
?>
...
<hr />
<h4 align="center"> DATA KEGIATAN DAN PEMBANGUNAN KESELURUHAN TAHUN ANGGARAN <?php echo date('Y')?></h4>					
	...
	<?php
		...
		if ($jum > 0) {
			...
			<tr>
				<td><?php echo $no++ ?></td>
				<td><?php if($no-1 == 1){ echo "Artesis Perencanaan";}?></td>
	...
	<?php
		...
		if ($jum > 0) {
			...
			<tr>
				<td><?php echo $no++ ?></td>
				<td><?php if($no-1 == 1){ echo "Artesis Terealisasi";}?></td>
				<td><?php echo $row->alamat ?></td>
				<td><?php echo $row->rt ?></td>
				<td><?php echo $row->rw ?></td>
				<td><?php echo isset($row->panjang) ? $row->panjang : '-';  ?></td>
